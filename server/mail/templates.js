...
<%} else {%>
Unfortunately there don't seem to be any orders with the email **"<%=email%>"**. Maybe try another? The email has to match what's on the order - it's the only way I can reconcile who ordered what.
<%} %>`


export const checkoutTemplate = `---
subject: "[Rob Conery] Thank you for your order: <%=checkout.number%>"
---

# Thanks for your order!

Your order number is **<%=checkout.number%>** and you can access your goodies below. Please be aware that links expire in the next 2 hours so please download!

<%if(checkout.invoice_url) {%>
## Your Invoice

You can see a copy if your receipt from my payment processor, Stripe, [which is right here](<%=checkout.invoice_url%>). If you need to add or change information, you should be able to do so on that page, or just edit the PDF as you need.
<%}%>
...
**<%=checkout.fulfillment.file %>** (<%=checkout.fulfillment.size%>)
[Download Now](<%=checkout.fulfillment.download_url%>)
                    ...
                    <tr>
                      <td>
                        <p style="text-align: center; padding: 20px">
                          <img
                            src="https://robconery.com/content/images/2022/08/robconery_logo-1.png"
                            alt=""
                            style="height: 50px"
                          />
                        </p>
